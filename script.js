...
const game = {
    ...
    pause() {
        ...
        // стопаем все выполнение, например так
        setTimeout(pausedGame, 200);
    },
    ...
    stop() {
        ...
        /* добавить сюда код */
        alert("Игра законченна. Набранных очков : " + score)
        board.clearBoard();
        score = 0;
    ...
    move(event) {
        ...
        if (foundFood !== -1) {
            ...
            /* если True то score +=1 */
            score += 1;
            /* если score > 0 то вывести очки на табло */
            if (score > 0) {
                document.getElementById('score-value').innerHTML = score;
        ...
        // if (snake.parts.length > document.getElementsByClassName('snake').length ) {
        //     game.stop();
        // }

        // console.log("Длинна змеи: " + snake.parts.length);
        let  snakeLength = snake.parts.length;
        // console.log("Кол-во клеток занимающее змеей: " + document.getElementsByClassName('snake').length)
        let snakeChain = document.getElementsByClassName('snake').length
        if (snakeLength > snakeChain){
            alert("Змея укусила себя за хвост.")
            game.stop();
        }    
    },
        ...
        // /** 
        //  (3. *) Научить змейку двигаться самой в заданном направлении (например, через функцию setInterval, setTimeout), как в классической игре.
        // */
        // play("Получить направление let directionMove = 'direction'"){
        //     повторить step = move(directionMove) каждые (config.speed) секунды
...
const board = {
    ...
    /**
     * Функция очищает поле с клетками для игры.
     */
    clearBoard() {
...
const food = {
    ...
    generateItem() {
            ...
            function createNewItem() {
                let top = getRandomNumber(0, config.size - 1);
                let left = getRandomNumber(0, config.size - 1);
                return {top, left}

            }
            let newfoodCoordinat = createNewItem();
            console.log("Создаем координату еды на поле", newfoodCoordinat)
            while (true){
            
            // добавить проверку нет ли у нас такого элемента

            // пока newItem === div с классом "food" или newItem === div с классом "snake"

            // генерировать новые координаты top и left
            let lastFoodCoordinat = document.querySelector(`.cell[data-top="${newfoodCoordinat.top}"][data-left="${newfoodCoordinat.left}"]`);
            console.log(lastFoodCoordinat);
            // foodCoordinat = получить координаты (top, left)

            // если координаты содержат:
            if (lastFoodCoordinat.classList.contains('snake') == true || lastFoodCoordinat.classList.contains('food') == true){
                console.log("Созданна не верная координата еды", newfoodCoordinat)
                
                newfoodCoordinat = createNewItem();
                console.log("персоздаем координаты еды на", newfoodCoordinat)
                
            } else {
                console.log('Добавим еду на поле', newfoodCoordinat)
